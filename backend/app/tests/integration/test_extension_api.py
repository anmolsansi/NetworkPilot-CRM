...
class TestDashboardAPI:
    ...
    async def test_due_requires_auth(self, client: AsyncClient):
        response = await client.get(
            "/api/v1/dashboard/due", params={"workspace_id": "00000000-0000-0000-0000-000000000000"}
        )
        assert response.status_code in [401, 422]
...
class TestTemplatesAPI:
    async def test_list_templates_requires_auth(self, client: AsyncClient):
        response = await client.get(
            "/api/v1/templates", params={"workspace_id": "00000000-0000-0000-0000-000000000000"}
        )
        assert response.status_code in [401, 422]
