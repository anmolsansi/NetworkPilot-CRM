...
class TestCrossWorkspaceAuthorization:
    async def test_cross_workspace_access_denied(self, client: AsyncClient, mock_headers: dict):
        ...
        resp = await client.post("/api/v1/workspaces", json={"name": "Workspace A"}, headers=mock_headers)
        ...
        resp = await client.post("/api/v1/workspaces", json={"name": "Workspace B"}, headers=user_b_headers)
        assert resp.status_code == 201
        workspace_b_id = resp.json()["id"]

        # User B tries to read Workspace A templates
        resp = await client.get(f"/api/v1/templates?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403

        # User B tries to create template in Workspace A
        resp = await client.post(f"/api/v1/templates?workspace_id={workspace_a_id}", json={
            "name": "Hacked",
            "category": "Test",
            "body": "Test"
        ...
        resp = await client.get(f"/api/v1/extension/lookup?workspace_id={workspace_a_id}&linkedin_url=test", headers=user_b_headers)
        assert resp.status_code == 403
        ...
        resp = await client.patch(f"/api/v1/workspaces/{workspace_a_id}", json={"name": "Hacked"}, headers=user_b_headers)
        assert resp.status_code == 403

        # User A creates a person in Workspace A
        resp = await client.post(f"/api/v1/people?workspace_id={workspace_a_id}", json={
            "name": "Person A",
        ...
        resp = await client.get(f"/api/v1/people?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403

        # User B tries to read a specific person in Workspace A
        resp = await client.get(f"/api/v1/people/{person_a_id}?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403
        ...
        resp = await client.patch(f"/api/v1/people/{person_a_id}?workspace_id={workspace_a_id}", json={
            "name": "Hacked Name"
        }, headers=user_b_headers)
        assert resp.status_code == 403

        # User B tries to delete a person in Workspace A
        resp = await client.delete(f"/api/v1/people/{person_a_id}?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403

        # User B tries to list activities for a person in Workspace A
        resp = await client.get(f"/api/v1/people/{person_a_id}/activities?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403
        ...
        resp = await client.get(f"/api/v1/dashboard/summary?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403

        # User B tries to get dashboard due in Workspace A
        resp = await client.get(f"/api/v1/dashboard/due?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403

        # User B tries to get calendar link in Workspace A
        resp = await client.get(f"/api/v1/calendar/daily-reminder-link?workspace_id={workspace_a_id}", headers=user_b_headers)
        assert resp.status_code == 403
