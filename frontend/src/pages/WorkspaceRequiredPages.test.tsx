...
describe('workspace-required pages', () => {
  ...
  it('applies column filters and sorting to People requests', async () => {
    ...
    const emailPresent = screen.getByLabelText('Email present')
    const emailMissing = screen.getByLabelText('Email missing')
    fireEvent.click(emailPresent)
    expect(emailPresent).toBeChecked()
    fireEvent.click(emailMissing)
    expect(emailMissing).toBeChecked()
    expect(emailPresent).not.toBeChecked()

    const invitePresent = screen.getByLabelText('Invite accepted values present')
    const inviteMissing = screen.getByLabelText('Invite accepted values missing')
    fireEvent.click(invitePresent)
    expect(invitePresent).toBeChecked()
    fireEvent.click(inviteMissing)
    expect(inviteMissing).toBeChecked()
    expect(invitePresent).not.toBeChecked()
    ...
    const listCalls = vi.mocked(peopleApi.list).mock.calls
    const appliedParams = listCalls[listCalls.length - 1]?.[0]
    expect(appliedParams).not.toHaveProperty('email_present')
    expect(appliedParams).not.toHaveProperty('invite_accepted_only')
